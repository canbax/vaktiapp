...
export class HijriDate {
  ...
  private readonly YEARS: Record<number, number[]> = {
    ...
    1447: [30, 29, 30, 30, 29, 30, 30, 30, 29, 29, 30, 29],
   ...
   * @param  {Date} d
   */
  getNearestSabbatical(after: Date): {
    cnt: number;
    sabbatical: Sabbatical | undefined;
    hijri: HijriDate;
    gregorian: Date;
  } {
    ...
    return {
      cnt: cnt,
      sabbatical: this.getSabbaticalObj4Date(hijri, tmpDate),
      hijri: hijri,
      gregorian: tmpDate,
    };
  }
  ...
  addDays(year: number, month: number, day: number, diff: number): HijriDate {
    ...
    while (diff != 0) {
      if (isPos) {
        ...
        if (day > 1) {
          // can decrease the day
          day--;
        } else if (month > 0) {
          // should decrease the month
          month--;
  ...
  dayDiff(date2: HijriDate): number {
    ...
    if (comparison == 3) {
      for (let y = smallDate.year; y < bigDate.year; y++) {
        daySum += this.YEARS[y].reduce((a, b) => {
          return a + b;
        }, 0);
      }
